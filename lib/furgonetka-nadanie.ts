...
export async function utworzPrzesylke(z: Zamowienie): Promise<WynikNadania> {
  const body = {
    ...
    type: "package",
    // Adres odbioru (skąd paczka jest nadawana) — Furgonetka wymaga pełnego
    // adresu (name/company/email/street/postcode/city/phone). U nas to ten
    // sam adres co nadawca.
    pickup: nadawca(),
    sender: nadawca(),
    receiver: odbiorca(z),
    parcels: [paczka(z)],
